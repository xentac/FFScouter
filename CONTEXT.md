...
### Settings Panel
The `ff-settings-panel` LitElement (`src/ui/settings-panel.ts`) injected beside `.profile-wrapper` on profile pages. Organized into four [[Settings Group]]s (API Key & Premium, [[Gauge Marker Settings]], Feature Toggles, Debug Settings), each laid out as its own per-section CSS grid that reflows safely within the [[Content Wrapper Width]]: plain selects and checkboxes are single grid cells (3-up at ≥784px, 1-up when narrow), while multi-control bundles (API key row, FF ranges, marker size + preview, color scheme + swatches, chain sub-options, prose notices) span the full grid width as their own rows. Selects stack their label above a cell-filling control; checkboxes stay inline control-first; nothing overflows the container at any of the three widths. The footer buttons (Save/Reset/Clear Cache) sit outside any group, after the last one.
