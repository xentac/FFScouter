...
if (apikey[0] != "#") {
  ...
  rD_setValue = (name, value) => {
    ffdebug("[FF Scouter V2] Attempted to set " + name);
    return localStorage.setItem(name, value);
  };
  rD_getValue = (name, defaultValue) => {
...
async function process_queue() {
  if (queued_player_ids.length > 0) {
    ...
    await process_queued_player_ids(processing_player_ids, () => {
...
const check_mutation = async (node) => {
  ...
  if (honor_bars.length > 0) {
    await apply_ff_gauge(Array.from(node.querySelectorAll(".honor-text-wrap")));
  } else {
...
async function getLocalUserId() {
  ...
  console.error("[FF Scouter V2] Could not extract user ID from profile link");
  return null;
}
